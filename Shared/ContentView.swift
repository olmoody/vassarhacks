// // ContentView.swift // VassarHacks App // // Created by Curtis Fedorko on 5/1/21. //
...
struct User: Decodable { //var id: ObjectIdentifier
...
class apiCall {
    func getUsers(completion:@escaping (Newss) -> ()) {
        ...
        request.httpMethod = "GET" // Set HTTP Request Header
        ...
        URLSession.shared.dataTask(with: request) { (data, _, _) in let users = try! JSONDecoder().decode(Newss.self, from: data!)
            print(users)
...
struct Actuals: Decodable { let vaccinationsCompleted: Int }
        ...
        NavigationView{
            VStack{
                ZStack{
                    ...
                    if image != nil{ image? .resizable() .scaledToFit() }
                    else { Text("Tap to Select a Photo") .foregroundColor(.white) .font(.headline) }
...
struct StatsState:View { //@State private var state : String? = nil
    var body: some View {
        NavigationView{
            List{
                NavigationLink(destination: Stats(state:"MA")) {
                    Text("Massachusetts") }.listRowBackground(Color.green)
                NavigationLink(destination: Stats(state:"NY")){
                    Text("New York") }.listRowBackground(Color.green)
                NavigationLink(destination: Stats(state:"CA")){
                    Text("California") }.listRowBackground(Color.green)
                NavigationLink(destination: Stats(state:"ND")){
                    Text("North Dakota") }.listRowBackground(Color.green)
...
struct Stats: View {
    ...
    { Color(UIColor.lightGray).edgesIgnoringSafeArea(.all)
        Text("Percent of people vaccinated in (state): (String(self.pop ?? 0))").font(.system(size: 35, weight: .bold, design: .default )) // Text(String(user.population)) // .font(.subheadline)
...
struct News:
    View {
    init() { UITableView.appearance().backgroundColor = .lightGray }
    @State var dic : String?
    @State var pop : Newss?
    //let a = apiCall(state:"MA");
    var state = "MA"
    //let dic = NSDictionary()
    var body: some View { NavigationView{ List(pop?.news ?? []) { user in //
        Link(user.title, destination: URL(string: user.link)!).font(.system(size: 15, weight: .bold, design: .default )).foregroundColor(Color.green) // .font(.headline) // Text(String(user.population)) // .font(.subheadline)
...
struct NavigateFromMenu: View {
    ...
    var body: some View {
        NavigationView {
            Menu {
                Button("AL") { openURL(URL(string: "https://www.alabamapublichealth.gov/covid19vaccine/index.html")!)
                    self.isActive = true }
                Button("AK") { openURL(URL(string: "http://dhss.alaska.gov/dph/epi/id/pages/COVID-19/vaccine.aspx")!)
                    self.isActive = true }
                Button("CT") { openURL(URL(string: "https://portal.ct.gov/Vaccine-Portal")!)
                    self.isActive = true }
                Button("WA") { openURL(URL(string: "https://www.doh.wa.gov/Emergencies/COVID19/vaccine")!)
                    self.isActive = true }
                Button("CA") { openURL(URL(string: "https://myturn.ca.gov/")!)
                    self.isActive = true }
                Button("NY") { openURL(URL(string: "https://am-i-eligible.covid19vaccine.health.ny.gov/")!)
                    self.isActive = true }
                Button("DE") { openURL(URL(string: "https://coronavirus.delaware.gov/vaccine/")!)
                    self.isActive = true }
                Button("ME") { openURL(URL(string: "https://www.maine.gov/dhhs/mecdc/infectious-disease/immunization/covid-19-providers/index.shtml")!)
                    self.isActive = true } }
                label: { Text("Select Your State") } .background( NavigationLink(destination: Text(self.navigateTo), isActive: $isActive) { EmptyView() }) } } }

struct UploadVax: UIViewControllerRepresentable { class Coordinator: NSObject, UINavigationControllerDelegate, UIImagePickerControllerDelegate { let parent: UploadVax
